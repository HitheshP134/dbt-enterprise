...
with orders as (
    select * from SALES.STAGE_stage.stg_orders
),
...
customers as (
    select
        ...
        revenue_tier,
        country
    from SALES.STAGE_dimensions.dim_customers
),
...
close_date as (
    select
        date_key as close_date_key,
        date_day as close_date_day
    from SALES.STAGE_dimensions.dim_date
),
...
final as (
    select
        -- surrogate key
        md5(cast(coalesce(cast(o.order_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as sales_key,
    ...
    inner join order_date as od on o.order_date = od.date_day
    inner join close_date as cd on o.close_date = cd.close_date_day
)
